...
interface Props {
  ...
  nome: string;
  avatarUrl: string;
  resumo: { total: number; minutos: number };
  comissao: number;
  ticketMedio: number;
  tipos: Record<string, number>;
  valoresPorTipo: Record<string, number>;
  tipoServicoIcone: Record<string, ReactNode>;
  totalMinutosMes: number;
}

export function ComissaoBarbeiroModal({
  isOpen, onClose, nome, avatarUrl, resumo, comissao, ticketMedio, tipos, valoresPorTipo, tipoServicoIcone, totalMinutosMes
}: Props) {
  return (
          ...
          <div className="flex flex-col gap-8 px-6 pb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-5 shadow flex flex-col items-center md:items-start">
                ...
                <ul className="text-base space-y-4 w-full">
                  {Object.entries(tipos).map(([tipo, qtd]) => (
                    <li key={tipo} className="flex items-center justify-between gap-4 w-full">
              ...
              <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-5 shadow flex flex-col items-center md:items-start">
                ...
                <ul className="text-base space-y-4 w-full">
                  {Object.entries(valoresPorTipo).map(([tipo, valor]) => (
                    <li key={tipo} className="flex items-center justify-between gap-4 w-full">
                      <span className="flex items-center gap-2">{tipoServicoIcone[tipo]} {tipo.charAt(0).toUpperCase() + tipo.slice(1)}</span>
