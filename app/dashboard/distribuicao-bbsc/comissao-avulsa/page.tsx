...
export default function ComissaoAvulsaPage() {
  ...
  useEffect(() => {
    if (form.unidade_id) fetchProfissionais(form.unidade_id);
  }, [form.unidade_id]);
  ...
  async function handleUnidadeChange(unidadeId: string) {
    setForm(f => ({ ...f, unidade_id: unidadeId, profissional_id: "", servico_avulso_id: "" }));
    await fetchProfissionais(unidadeId);
    await fetchServicos(unidadeId);
    setStep(2);
  }

  async function handleProfissionalChange(profissionalId: string) {
    setForm(f => ({ ...f, profissional_id: profissionalId }));
    setStep(3);
  }

  async function fetchProfissionais(unidadeId: string) {
  ...
  async function fetchUnidades() {
    ...
    setUnidades(data || []);
    if (data && data.length > 0 && !form.unidade_id) setForm(f => ({ ...f, unidade_id: data[0].id }));
  }
  async function fetchServicos(unidadeId: string) {
  ...
  const BARBERBEER_ID = '87884040-cafc-4625-857b-6e0402ede7d7';
  const barbeirosBarberBeer = profissionais.filter((p: any) => p.unidade_id === BARBERBEER_ID && p.funcao === 'barbeiro');
  const comissoesPorProfissional = barbeirosBarberBeer.map((p: any) => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 pb-10">
      ...
      <div className="max-w-7xl mx-auto px-4 mt-10 flex flex-col md:flex-row gap-10">
                ...
                >
                  <Card.Header className="flex items-center gap-3 pb-0">
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-center">
                      <span className="font-bold text-base text-gray-800 group-hover:text-blue-700 transition-colors">{p.nome?.split(" ")[0]}</span>
        ...
        <div className="flex-1">
          <h2 className="text-xl font-bold text-gray-800 mb-6 tracking-tight">Serviços Avulsos de <span className="text-blue-700">{profissionalAtivo?.nome || "-"}</span></h2>
          {profissionalAtivo && Array.isArray(profissionalAtivo.comissoes) ? (
            <>
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 mb-6">
                <div className="flex gap-6 flex-wrap">
                  <span className="text-lg text-gray-700">Valor total: <span className="font-bold text-blue-700 text-xl">R$ {profissionalAtivo?.valorTotal?.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</span></span>
                  <span className="text-lg text-gray-700">Minutos totais: <span className="font-bold text-blue-700 text-xl">{profissionalAtivo?.tempoTotal}</span></span>
                </div>
                  ...
                  <tbody>
                    {profissionalAtivo.comissoes.length === 0 ? (
                      ...
                      profissionalAtivo.comissoes.map((c: any) => (
                        <tr key={c.id} className="bg-white hover:bg-blue-50 transition">
