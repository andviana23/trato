...
export default function AgendamentosPage() {
  ...
  const daysOfWeek = useMemo(() => {
    const start = dayjs(data).startOf('week');
    ...
    <DashboardLayout>
      ...
      <div className="flex items-center justify-between mb-2 mt-2">
        <div className="flex items-center gap-2">
          <Button size="icon" variant="ghost" onClick={() => setData(dayjs(data).add(-1,'day').format('YYYY-MM-DD'))}><ChevronLeftIcon className="w-5 h-5"/></Button>
          <div className="text-lg font-medium text-gray-700">{dayjs(data).format('MMMM YYYY')}</div>
          <Button size="icon" variant="ghost" onClick={() => setData(dayjs(data).add(1,'day').format('YYYY-MM-DD'))}><ChevronRightIcon className="w-5 h-5"/></Button>
        </div>
        <div />
      </div>
