...
import { Card, CardBody, CardHeader, Chip, Avatar, Button, Input, Modal, ModalContent, ModalHeader, ModalBody, ModalFooter } from "@nextui-org/react";
...
export default function PaginaServicos() {
  ...
  const [servicoEdit, setServicoEdit] = useState<any | null>(null);
  const [servicoDelete, setServicoDelete] = useState<any | null>(null);
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} size="sm">
          <ModalContent>
            ...
            <ModalBody>
              <Input label="Nome do Serviço" value={form.nome} onChange={e => setForm(f => ({ ...f, nome: e.target.value }))} isRequired />
              <Input label="Tempo de Execução (minutos)" type="number" value={form.tempo_minutos} onChange={e => setForm(f => ({ ...f, tempo_minutos: e.target.value }))} isRequired min={1} />
        ...
        <Modal isOpen={modalDelete} onClose={() => setModalDelete(false)} size="sm">
          <ModalContent>
            ...
            <ModalBody>
              <p>Tem certeza que deseja excluir o serviço <span className="font-semibold">{servicoDelete?.nome}</span>?</p>
            </ModalBody>
            <ModalFooter>
