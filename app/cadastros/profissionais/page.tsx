...
type Profissional = {
  ...
  avatar_url?: string;
  unidade_id?: string;
  user_id?: string; // Adicionado para armazenar o ID do usuário no Supabase Auth
  [key: string]: any;
};
...
export default function PaginaProfissionais() {
  ...
  const [profissionalDelete, setProfissionalDelete] = useState<Profissional | null>(null);
  ...
  const [unidadeEdit, setUnidadeEdit] = useState<Unidade | null>(null);
  ...
  async function fetchProfissionais() {
    setLoading(true);
    if (!unidadeSelecionada) { setProfissionais([]); setLoading(false); return; }
  ...
  function openDelete(p: Profissional) {
  ...
  async function fetchUnidades() {
    ...
    setUnidades(data || []);
    if (!unidadeSelecionada && data && data.length > 0) setUnidadeSelecionada(data[0].id);
  }
  ...
  function openEditarUnidade(u: Unidade) {
    setUnidadeEdit(u);
  ...
  async function salvarUnidade() {
    ...
    setErroUnidade("");
    if (!formUnidade.nome) { setErroUnidade("Nome obrigatório"); setSavingUnidade(false); return; }
    if (unidadeEdit) {
      const { error } = await supabase.from("unidades").update(formUnidade).eq("id", unidadeEdit.id);
      if (error) { setErroUnidade(error.message); setSavingUnidade(false); return; }
    } else {
  ...
  async function redefinirSenha() {
    if (!profissionalEdit?.user_id || !novaSenha) {
      ...
      <div className="container mx-auto max-w-6xl px-2 md:px-0 py-8">
              ...
              <Table aria-label="Lista de profissionais" removeWrapper className="min-w-full text-sm">
                ...
                <TableBody emptyContent={loading ? "Carregando..." : "Nenhum profissional cadastrado"}>
                  {profissionais.filter(p =>
                    ...
                    <TableRow key={p.id} className="hover:bg-blue-50 dark:hover:bg-gray-800 transition-colors">
                      <TableCell>
          ...
          <ModalContent>
            ...
            <ModalBody className="space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Input label="Nome" value={form.nome} onChange={e => setForm(f => ({ ...f, nome: e.target.value }))} isRequired startContent={<UserIcon className="w-4 h-4" />} size="sm" radius="sm" variant="bordered" />
                <Input label="Telefone" value={form.telefone} onChange={e => setForm(f => ({ ...f, telefone: e.target.value }))} isRequired startContent={<PhoneIcon className="w-4 h-4" />} size="sm" radius="sm" variant="bordered" />
                <Input label="Email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} isRequired startContent={<EnvelopeIcon className="w-4 h-4" />} size="sm" radius="sm" variant="bordered" />
                <Input label="Senha" type="password" value={form.senha} onChange={e => setForm(f => ({ ...f, senha: e.target.value }))} isRequired={!profissionalEdit} size="sm" radius="sm" variant="bordered" />
                <Input label="Data de Nascimento" type="date" value={form.data_nascimento} onChange={e => setForm(f => ({ ...f, data_nascimento: e.target.value }))} isRequired={false} size="sm" radius="sm" variant="bordered" />
              </div>
              <RadioGroup label="Função" value={form.funcao} onValueChange={v => setForm(f => ({ ...f, funcao: v }))} orientation="horizontal" className="mt-2">
              ...
              {profissionalEdit && (
                <div className="flex flex-col gap-2 mt-2">
                  ...
                  {!showResetSenha ? (
                    <Button color="warning" variant="bordered" onClick={() => setShowResetSenha(true)}>
                      Redefinir Senha
                    </Button>
                  ) : (
        ...
        <Modal isOpen={modalUnidade} onClose={() => setModalUnidade(false)} size="md">
          <ModalContent>
            ...
            <ModalBody className="space-y-3">
              <Input label="Nome da Unidade" value={formUnidade.nome} onChange={e => setFormUnidade(f => ({ ...f, nome: e.target.value }))} isRequired startContent={<BuildingOffice2Icon className="w-4 h-4" />} size="sm" radius="sm" variant="bordered" />
              <Input label="Endereço" value={formUnidade.endereco} onChange={e => setFormUnidade(f => ({ ...f, endereco: e.target.value }))} size="sm" radius="sm" variant="bordered" />
              <Input label="Telefone" value={formUnidade.telefone} onChange={e => setFormUnidade(f => ({ ...f, telefone: e.target.value }))} size="sm" radius="sm" variant="bordered" />
        ...
        <Modal isOpen={modalDelete} onClose={() => setModalDelete(false)} size="sm">
          <ModalContent>
            ...
            <ModalBody>
              <p className="text-base">Tem certeza que deseja excluir o profissional <span className="font-semibold">{profissionalDelete?.nome}</span>?</p>
            </ModalBody>
            <ModalFooter className="border-t pt-3">
