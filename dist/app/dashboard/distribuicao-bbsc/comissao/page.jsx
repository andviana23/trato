...
import { Card, CardBody, Button, Modal, ModalContent, ModalHeader, ModalBody, ModalFooter, Select, SelectItem, Input } from "@nextui-org/react";
...
export default function ComissaoPage() {
    ...
    const [profissionaisAvulso, setProfissionaisAvulso] = useState([]);
    ...
    const [comissoesConsulta, setComissoesConsulta] = useState([]);
    const [modalListaOpen, setModalListaOpen] = useState(false);
    const [profissionaisTodos, setProfissionaisTodos] = useState([]);
    const [loadingConsulta, setLoadingConsulta] = useState(false);
    const [modalComparativoOpen, setModalComparativoOpen] = useState(false);
    const [barbeiroComparativo, setBarbeiroComparativo] = useState(null);
    useEffect(() => { fetchAll(); }, []);
    useEffect(() => { if (unidadeAvulso) {
        fetchProfissionaisAvulso();
    ...
    async function fetchUnidadesAvulso() {
        ...
        setUnidades(data || []);
        if (data && data.length > 0 && !unidadeAvulso)
            setUnidadeAvulso(data[0].id);
    }
    async function fetchProfissionaisAvulso() {
        ...
        if (data && data.length > 0)
            setProfissionalAvulso(data[0].id);
    }
    ...
    async function excluirComissaoAvulsa(id) {
    ...
    async function fetchProfissionaisTodos() {
        ...
        if (data && data.length > 0)
            setProfissionalConsulta(data[0].id);
    }
    ...
    return (<div className="min-h-screen bg-gray-50 pb-10">
      ...
      <div className="max-w-7xl mx-auto px-4 mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
        <ComissaoResumoCard icon={<CurrencyDollarIcon className="w-7 h-7 text-green-600"/>} title="Faturamento Total do Mês" value={faturamento} valueColorClass="text-green-700" gradientClass="bg-gradient-to-br from-green-100 to-white"/>
        <ComissaoResumoCard icon={<ChartBarIcon className="w-7 h-7 text-amber-500"/>} title="Comissão Total do Mês (40%)" value={comissaoTotal} valueColorClass="text-amber-700" gradientClass="bg-gradient-to-br from-amber-100 to-white"/>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 mt-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {barbeiros.length === 0 ? (<Card className="shadow-md rounded-2xl col-span-full">
            ...
            return (<ComissaoBarbeiroCard key={b.id} nome={b.nome} avatarUrl={b.avatar_url} minutos={minutos} percentual={percentual} comissao={comissaoTotalBarbeiro} ticketMedio={ticketMedio} tipos={tipos} tipoServicoIcone={tipoServicoIcone} onClick={() => setBarbeiroDetalhe(b)}/>);
        }))}
        </div>
      </div>
      ...
      <Modal isOpen={!!barbeiroDetalhe} onClose={() => setBarbeiroDetalhe(null)} size="xl" isDismissable closeButton className="z-[9999]">
        <ModalContent className="!p-0">
          <ModalHeader className="text-center text-2xl font-bold text-gray-900 pb-0 pt-6">Detalhes da Comissão de {(barbeiroDetalhe === null || barbeiroDetalhe === void 0 ? void 0 : barbeiroDetalhe.nome) || 'N/A'}</ModalHeader>
      ...
      <Modal isOpen={modalAvulsoOpen} onClose={() => setModalAvulsoOpen(false)} size="md">
        <ModalContent>
          ...
          <ModalBody>
            <Select label="Unidade da Barbearia" selectedKeys={unidadeAvulso ? [unidadeAvulso] : []} onChange={e => setUnidadeAvulso(e.target.value)} isRequired className="mb-2">
              {unidades.map((u) => (<SelectItem key={u.id} value={u.id}>{u.nome}</SelectItem>))}
            </Select>
            <Select label="Profissional" selectedKeys={profissionalAvulso ? [profissionalAvulso] : []} onChange={e => setProfissionalAvulso(e.target.value)} isRequired className="mb-2">
              {profissionaisAvulso.map((p) => (<SelectItem key={p.id} value={p.id}>{p.nome}</SelectItem>))}
            </Select>
            <Select label="Serviço Realizado" selectedKeys={servicoAvulso ? [servicoAvulso] : []} onChange={e => setServicoAvulso(e.target.value)} isRequired className="mb-2">
              {servicosAvulsos.map((s) => (<SelectItem key={s.id} value={s.id}>{s.nome}</SelectItem>))}
            </Select>
      ...
      <Modal isOpen={modalConsultaOpen} onClose={() => setModalConsultaOpen(false)} size="sm">
        <ModalContent>
          ...
          <ModalBody>
            <Select label="Profissional" selectedKeys={profissionalConsulta ? [profissionalConsulta] : []} onChange={e => setProfissionalConsulta(e.target.value)} isRequired className="mb-2">
              {profissionaisTodos.map((p) => (<SelectItem key={p.id} value={p.id}>{p.nome}</SelectItem>))}
            </Select>
            <Input label="Data" type="date" value={dataConsulta} onChange={e => setDataConsulta(e.target.value)} isRequired className="mb-2"/>
          </ModalBody>
          <ModalFooter>
          ...
          <ModalBody>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 text-sm">
                ...
                <tbody>
                  {comissoesConsulta.length === 0 ? (<tr><td colSpan={5} className="text-center py-8 text-gray-400">Nenhuma comissão avulsa encontrada.</td></tr>) : (comissoesConsulta.map((c) => {
            var _a, _b;
            return (<tr key={c.id} className="bg-white">
                        <td className="px-4 py-3">{((_a = c.servicos_avulsos) === null || _a === void 0 ? void 0 : _a.nome) || '-'}</td>
                        ...
                        <td className="px-4 py-3 text-center">{((_b = c.servicos_avulsos) === null || _b === void 0 ? void 0 : _b.tempo_minutos) || '-'}</td>
