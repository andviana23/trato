...
import { useFaturamentoAssinatura } from './useFaturamentoAssinatura';
export default function FaturamentoAssinaturaPage() {
    ...
    return (<>
      ...
      <form onSubmit={handleAdicionar} className="flex flex-col md:flex-row gap-4 items-stretch md:items-end mb-8">
        <div className="flex-1">
          <Input type="number" label="Valor" placeholder="0,00" value={valor} onChange={e => setValor(e.target.value)} min={0} step={0.01} required startContent={<CurrencyDollarIcon className="w-5 h-5 text-amber-400"/>}/>
        </div>
        <div className="flex-1">
          <Input type="text" label="Descrição" placeholder="Ex: Corte, Barba, etc." value={descricao} onChange={e => setDescricao(e.target.value)} required/>
        </div>
        <div className="flex-1">
          <Input type="date" label="Data" value={data} onChange={e => setData(e.target.value)} required/>
        </div>
        <Button type="submit" color="warning" className="w-full md:w-auto font-bold" startContent={<PlusIcon className="w-5 h-5"/>}>
      ...
      <Card className="shadow border border-amber-200">
        ...
        <CardBody>
          {loading ? (<div className="text-center text-amber-500">Carregando...</div>) : erro ? (<div className="text-center text-red-500">{erro}</div>) : lancamentos.length === 0 ? (<div className="text-center text-amber-500">Nenhum lançamento adicionado ainda.</div>) : (<div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-amber-200 dark:divide-amber-700">
                ...
                <tbody>
                  {lancamentos.map(l => (<tr key={l.id} className="hover:bg-amber-50 dark:hover:bg-amber-900">
                      ...
                      <td className="px-4 py-2 whitespace-nowrap text-right">
                        <button type="button" onClick={() => handleExcluir(l.id)} className="inline-flex items-center justify-center p-1 rounded hover:bg-red-100 dark:hover:bg-red-900 transition-colors" title="Excluir">
