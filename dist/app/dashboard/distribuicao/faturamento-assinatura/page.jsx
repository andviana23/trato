    ...
    return (<>
      ...
      <form onSubmit={handleAdicionar} className="flex flex-col md:flex-row gap-4 items-stretch md:items-end mb-8">
        <div className="flex-1">
          <Input type="number" label="Valor" placeholder="0,00" value={valor} onChange={e => setValor(e.target.value)} min={0} step={0.01} required startContent={<CurrencyDollarIcon className="w-5 h-5 text-gray-400"/>}/>
        </div>
        <div className="flex-1">
        ...
        <Button type="submit" color="primary" className="w-full md:w-auto" startContent={<PlusIcon className="w-5 h-5"/>}>
      ...
      <Card className="shadow">
        ...
        <CardBody>
          {loading ? (<div className="text-center text-gray-500">Carregando...</div>) : erro ? (<div className="text-center text-red-500">{erro}</div>) : lancamentos.length === 0 ? (<div className="text-center text-gray-500">Nenhum lançamento adicionado ainda.</div>) : (<div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody>
                  {lancamentos.map(l => (<tr key={l.id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
