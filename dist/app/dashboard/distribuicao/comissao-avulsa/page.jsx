...
export default function ComissaoAvulsaTratoPage() {
    ...
    const comissoesPorProfissional = barbeiros.map((p) => {
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-gray-50 to-green-50 pb-10">
      <div className="bg-white shadow-sm sticky top-0 z-10 border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 py-6 flex flex-col md:flex-row md:items-center md:justify-between gap-2">
          ...
          <div className="flex gap-4 items-center">
            <input type="month" value={mesSelecionado} onChange={e => { setMesSelecionado(e.target.value); fetchComissoesPorMes(e.target.value); }} className="border border-gray-300 rounded-lg px-3 py-2 text-base focus:outline-none focus:ring-2 focus:ring-green-400 bg-white shadow-sm" style={{ minWidth: 140 }}/>
      ...
      <div className="max-w-7xl mx-auto px-4 mt-10 flex flex-col md:flex-row gap-10">
        ...
        <div className="w-full md:w-1/4 flex-shrink-0">
          ...
          <div className="flex flex-col gap-5">
            ...
            var _a;
            return (<Card key={p.id} isPressable onPress={() => setProfissionalSelecionado(p.id)} className={`rounded-2xl shadow-md border-2 transition-all duration-200 ${profissionalSelecionado === p.id || (!profissionalSelecionado && p === comissoesPorProfissional[0]) ? "border-green-600 bg-green-50 scale-[1.03]" : "border-gray-100 bg-white hover:scale-[1.01] hover:shadow-lg"} cursor-pointer group`}>
                  <CardHeader className="flex items-center gap-3 pb-0">
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-center">
                      <span className="font-bold text-base text-gray-800 group-hover:text-green-700 transition-colors">{(_a = p.nome) === null || _a === void 0 ? void 0 : _a.split(" ")[0]}</span>
        ...
        <div className="flex-1">
          <h2 className="text-xl font-bold text-gray-800 mb-6 tracking-tight">Serviços Avulsos de <span className="text-green-700">{(profissionalAtivo === null || profissionalAtivo === void 0 ? void 0 : profissionalAtivo.nome) || "-"}</span></h2>
          {profissionalAtivo && Array.isArray(profissionalAtivo.comissoes) ? (<>
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 mb-6">
                <div className="flex gap-6 flex-wrap">
                  <span className="text-lg text-gray-700">Valor total: <span className="font-bold text-green-700 text-xl">R$ {(_a = profissionalAtivo === null || profissionalAtivo === void 0 ? void 0 : profissionalAtivo.valorTotal) === null || _a === void 0 ? void 0 : _a.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</span></span>
                  <span className="text-lg text-gray-700">Minutos totais: <span className="font-bold text-green-700 text-xl">{profissionalAtivo === null || profissionalAtivo === void 0 ? void 0 : profissionalAtivo.tempoTotal}</span></span>
                </div>
              ...
              <div className="overflow-x-auto bg-white rounded-2xl shadow-lg border border-gray-100">
                ...
                var _a, _b;
                return (<tr key={c.id} className="bg-white hover:bg-green-50 transition">
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} size="md">
        <ModalContent>
          ...
          <ModalBody>
            {step === 1 && (<Select label="Profissional" selectedKeys={form.profissional_id ? [form.profissional_id] : []} onChange={e => { setForm(f => (Object.assign(Object.assign({}, f), { profissional_id: e.target.value }))); setStep(2); }} isRequired className="mb-2">
