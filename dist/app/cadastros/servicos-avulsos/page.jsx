...
export default function ServicosAvulsosPage() {
    ...
    async function fetchUnidades() {
        ...
        setUnidades(data || []);
        if (data && data.length > 0 && !unidadeSelecionada)
    ...
    return (<LayoutCadastros titulo="Serviços Avulsos">
      <div className="max-w-3xl mx-auto w-full">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div className="flex items-center gap-2">
            <span className="font-semibold text-blue-700">Unidade:</span>
            <Select aria-label="Selecionar unidade" selectedKeys={unidadeSelecionada ? [unidadeSelecionada] : []} onChange={e => setUnidadeSelecionada(e.target.value)} className="min-w-[180px]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {servicos.length === 0 ? (<Card className="shadow-md col-span-full">
              <CardBody className="text-center text-gray-400 py-12">Nenhum serviço avulso cadastrado para esta unidade.</CardBody>
            </Card>) : (servicos.map((servico) => (<Card key={servico.id} className="hover:shadow-lg transition-shadow shadow-lg">
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} size="sm">
        <ModalContent>
          ...
          <ModalBody>
            <Select label="Unidade da Barbearia" selectedKeys={form.unidade_id ? [form.unidade_id] : []} onChange={e => setForm(f => (Object.assign(Object.assign({}, f), { unidade_id: e.target.value })))} isRequired className="mb-2">
              {unidades.map((u) => (<SelectItem key={u.id} value={u.id}>{u.nome}</SelectItem>))}
            </Select>
            <Input label="Nome do Serviço" value={form.nome} onChange={e => setForm(f => (Object.assign(Object.assign({}, f), { nome: e.target.value })))} isRequired className="mb-2"/>
            <Input label="Tempo de Execução (minutos)" type="number" value={form.tempo_minutos} onChange={e => setForm(f => (Object.assign(Object.assign({}, f), { tempo_minutos: e.target.value })))} isRequired min={1} className="mb-2"/>
            <Input label="Valor do Serviço (R$)" type="number" value={form.valor} onChange={e => setForm(f => (Object.assign(Object.assign({}, f), { valor: e.target.value })))} isRequired min={0} step={0.01} className="mb-2"/>
