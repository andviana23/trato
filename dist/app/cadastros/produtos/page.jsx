...
export default function PaginaProdutos() {
    ...
    const [modalEditar, setModalEditar] = useState({ open: false });
    ...
    // Filtro de produtos
    const produtosFiltrados = produtos.filter((p) => p.nome.toLowerCase().includes(search.toLowerCase()) ||
    ...
    async function salvarEdicao(produtoEditado) {
    ...
    const marcasResumo = produtos.reduce((acc, p) => {
        if (!acc[p.marca])
            acc[p.marca] = { quantidade: 0, valorTotal: 0 };
    ...
    return (<div className="max-w-7xl mx-auto p-4">
      ...
      <div className="flex flex-wrap gap-2 mb-6 items-center">
        ...
        {UNIDADES.map((u) => (<Button key={u.label} variant={unidadeSelecionada.label === u.label ? "default" : "outline"} className={unidadeSelecionada.label === u.label
                ? "bg-blue-600 text-white border-blue-600 hover:bg-blue-700"
                : "bg-white text-blue-600 border-blue-600 hover:bg-blue-50"} onClick={() => { setUnidadeSelecionada(u); setPage(1); }}>
            {u.label}
          </Button>))}
      </div>
      {/* Topo com botões */}
      ...
      <div className="flex flex-wrap items-center gap-2 mb-2 bg-gray-50 rounded px-3 py-2 shadow-sm">
        ...
        <div className="ml-auto flex items-center gap-2">
          <select className="border rounded px-2 py-1 text-sm focus:ring focus:ring-blue-200" value={perPage} onChange={e => { setPerPage(Number(e.target.value)); setPage(1); }}>
            {[10, 20, 50, 100].map(n => <option key={n} value={n}>{n}</option>)}
          </select>
          <span className="text-sm">resultados por página</span>
          <Input className="w-48 border focus:ring focus:ring-blue-200" placeholder="Pesquisar" value={search} onChange={e => { setSearch(e.target.value); setPage(1); }}/>
        </div>
      </div>
      {/* Tabela de produtos */}
      <div className="overflow-x-auto bg-white rounded shadow mt-4">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {produtosPaginados.length === 0 && (<tr>
                <td colSpan={7} className="text-center p-4">Nenhum produto cadastrado.</td>
              </tr>)}
            {produtosPaginados.map((p) => (<tr key={p.id} className="border-t hover:bg-blue-50 transition-colors">
      ...
      <div className="flex flex-wrap items-center justify-between mt-4">
        ...
        <div className="flex gap-1">
          ...
          {[...Array(totalPages)].map((_, i) => (<Button key={i} size="sm" variant={page === i + 1 ? "default" : "outline"} onClick={() => setPage(i + 1)}>
              {i + 1}
            </Button>))}
      ...
      {/* Modal de edição */}
      {modalEditar.open && modalEditar.produto && (<ModalEditarProduto produto={modalEditar.produto} onClose={() => setModalEditar({ open: false })} onSave={salvarEdicao}/>)}
      {/* Modal de marcas */}
      {modalMarcas && (<ModalMarcas marcasResumo={marcasResumo} onClose={() => setModalMarcas(false)}/>)}
    </div>);
}
// Modal de edição de produto
function ModalEditarProduto({ produto, onClose, onSave }) {
    const [form, setForm] = useState(Object.assign({}, produto));
    return (<div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-lg relative" onClick={e => e.stopPropagation()}>
        ...
        <form className="space-y-3" onSubmit={e => { e.preventDefault(); onSave(form); }}>
          <Input placeholder="Nome do produto" value={form.nome} onChange={e => setForm(Object.assign(Object.assign({}, form), { nome: e.target.value }))} disabled/>
          <Input placeholder="Categoria" value={form.categoria} onChange={e => setForm(Object.assign(Object.assign({}, form), { categoria: e.target.value }))} required/>
          <Input placeholder="Marca" value={form.marca} onChange={e => setForm(Object.assign(Object.assign({}, form), { marca: e.target.value }))} required/>
          <Input type="number" placeholder="Valor (R$)" value={form.valor} onChange={e => setForm(Object.assign(Object.assign({}, form), { valor: Number(e.target.value) }))} required min={0}/>
          <Input type="number" placeholder="Comissão (%)" value={form.comissao} onChange={e => setForm(Object.assign(Object.assign({}, form), { comissao: Number(e.target.value) }))} required min={0} max={1} step={0.01}/>
...
// Modal de marcas
function ModalMarcas({ marcasResumo, onClose }) {
    return (<div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-lg relative" onClick={e => e.stopPropagation()}>
        <button className="absolute top-2 right-2 text-gray-400 hover:text-gray-600" onClick={onClose}>×</button>
        ...
        <table className="min-w-full text-sm mb-4">
          ...
          <tbody>
            {Object.entries(marcasResumo).map(([marca, info]) => (<tr key={marca} className="border-t">
