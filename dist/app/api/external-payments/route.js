...
export async function GET(request) {
    ...
    try {
        ...
        console.log('🔍 [API] Variáveis de ambiente:', {
            ...
            anon: ((_a = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY) === null || _a === void 0 ? void 0 : _a.substring(0, 8)) + '...'
        });
        ...
        const paymentsWithClient = await Promise.all(data.map(async (payment) => {
            ...
            return Object.assign(Object.assign({}, payment), { customerName,
                customerEmail, value: payment.price, status: calculateStatus(payment.current_period_end), daysOverdue: calculateDaysOverdue(payment.current_period_end) });
        }));
        ...
        return NextResponse.json({ success: false, error: 'Erro ao buscar pagamentos externos', details: (error === null || error === void 0 ? void 0 : error.message) || error }, { status: 500 });
    }
}
function calculateStatus(periodEnd) {
    const end = new Date(periodEnd);
    const today = new Date();
    const daysDiff = Math.floor((today.getTime() - end.getTime()) / (1000 * 60 * 60 * 24));
    return daysDiff > 0 ? 'ATRASADO' : 'ATIVO';
}
function calculateDaysOverdue(periodEnd) {
    const end = new Date(periodEnd);
    const today = new Date();
