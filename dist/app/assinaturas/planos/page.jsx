...
export default function PlanosPage() {
    ...
    useEffect(() => {
        async function fetchPlanos() {
            ...
            try {
                ...
                setPlanos(data || []);
                setCategorias(Array.from(new Set((data === null || data === void 0 ? void 0 : data.map((p) => p.categoria)) || [])));
            }
            catch (e) {
    ...
    async function handleNovoPlano(plano) {
        try {
            const novo = await criarPlano(plano);
            if (!novo)
                throw new Error('Plano não foi criado.');
    ...
    async function handleEditarPlano(plano) {
        setPlanoEditando(plano);
        setModalAberto(true);
    }
    async function handleSalvarPlanoEditado(planoEditado) {
    ...
    async function handleExcluirPlano(planoId) {
        if (window.confirm('Tem certeza que deseja excluir este plano?')) {
            // await excluirPlano(planoId); // implementar backend
            setPlanos((prev) => prev.filter((p) => p.id !== planoId));
        }
    }
    const planosFiltrados = planos.filter((plano) => categoriaFiltro === "" || plano.categoria === categoriaFiltro);
      ...
      <div>
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          ...
          <div className="flex items-center gap-2 bg-zinc-50 dark:bg-zinc-800 rounded-lg px-4 py-2 border border-zinc-200 dark:border-zinc-700">
            ...
            <select id="filtro-categoria-global" value={categoriaFiltro} onChange={e => setCategoriaFiltro(e.target.value)} className="px-3 py-2 rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-zinc-800 dark:text-zinc-100 focus:outline-none">
              <option value="">Todas</option>
              {categorias.map((cat) => (<option key={cat} value={cat}>{cat}</option>))}
            </select>
          </div>
          <button className="bg-black hover:bg-zinc-800 text-white font-semibold px-6 py-2 rounded-lg shadow transition-colors" onClick={() => setModalAberto(true)}>
            Novo Plano
          </button>
        </div>

        {/* Mensagem de erro */}
        {erro && (<div className="mb-6">
        ...
        {/* Conteúdo principal */}
        {carregando ? (<div className="flex justify-center items-center h-32">
            <span className="text-zinc-500 dark:text-zinc-300">Carregando planos...</span>
          </div>) : (<div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-md p-6">
            <h2 className="text-xl md:text-2xl font-bold text-zinc-800 dark:text-zinc-100 mb-6">Planos cadastrados</h2>
            <ul className="divide-y divide-zinc-100 dark:divide-zinc-800">
              {planosFiltrados.map((plano) => (<li key={plano.id} className="py-3 flex flex-col md:flex-row md:items-center md:justify-between gap-2 text-sm md:text-base">
                  <div className="flex-1 min-w-0">
                    <span className="font-semibold text-zinc-800 dark:text-zinc-100 text-base md:text-lg">{plano.nome}</span>
                    {plano.categoria && (<span className="ml-2 text-xs px-2 py-0.5 rounded bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 font-medium">{plano.categoria}</span>)}
