...
export default function MenuLateral() {
    ...
    if (isRecepcionista) {
        return (<nav className="flex flex-col gap-2 p-4">
        <Link href="/assinaturas/assinantes" passHref legacyBehavior>
          <a className={`flex items-center gap-3 px-4 py-2 rounded-lg font-medium transition-all duration-150
    ...
    return (<nav className="flex flex-col gap-2 p-4">
      <Link href="/assinaturas" passHref legacyBehavior>
        <a className={`flex items-center gap-3 px-4 py-2 rounded-lg font-medium transition-all duration-150
            ${pathname === '/assinaturas' ? 'bg-blue-600 text-white shadow' : 'text-gray-700 hover:bg-gray-100'}`} aria-current={pathname === '/assinaturas' ? 'page' : undefined}>
          <FiFileText className="w-5 h-5"/>
