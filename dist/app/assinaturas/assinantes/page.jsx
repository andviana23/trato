...
export default function AssinantesPage() {
    ...
    const [modalConfirmar, setModalConfirmar] = useState({ open: false, assinatura: null });
    useEffect(() => {
        getAssinaturas().then((assinaturas) => setAssinaturas(assinaturas));
    }, []);
    // Unir pagamentos_asaas e assinaturas (dinheiro) - agora com useMemo
    const todosAssinantes = useMemo(() => [
        ...pagamentos.map((p) => ({
            id: p.payment_id,
    ...
    useEffect(() => {
        ...
        if (busca) {
            assinantesFiltrados = assinantesFiltrados.filter(assinante => { var _a; return (_a = assinante.nome) === null || _a === void 0 ? void 0 : _a.toLowerCase().includes(busca.toLowerCase()); });
        }
        if (periodo.dataInicio || periodo.dataFim) {
            assinantesFiltrados = assinantesFiltrados.filter(assinante => {
                ...
                if (inicio && fim)
                    return data.isSameOrAfter(inicio) && data.isSameOrBefore(fim);
                if (inicio)
                    return data.isSameOrAfter(inicio);
                if (fim)
                    return data.isSameOrBefore(fim);
                return true;
            });
        }
    ...
    const pagamentosExternos = assinantesFiltrados.filter(a => a.origem === 'dinheiro' && (a.status === 'AGUARDANDO_PAGAMENTO' || a.status === 'CONFIRMED'));
      ...
      <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-10 font-sans">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
          ...
          <div className="flex gap-2">
            <button className={`bg-gray-100 text-gray-700 px-4 py-2 rounded-lg font-semibold shadow hover:bg-gray-200 transition-all flex items-center gap-2 ${isSyncing ? 'opacity-60 cursor-not-allowed' : ''}`} onClick={handleSyncPagamentos} disabled={isSyncing}>
              {isSyncing && (<svg className="animate-spin w-5 h-5 text-gray-500" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"></path></svg>)}
              Atualizar Dados
            </button>
        ...
        <div className="flex flex-wrap gap-4 items-end bg-white rounded-xl shadow p-4 mb-4">
          <input type="text" placeholder="Buscar por nome do cliente" value={busca} onChange={e => setBusca(e.target.value)} className="w-64 px-3 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500"/>
          <div className="flex items-center gap-2">
            <label className="text-xs text-gray-500">Período de pagamento</label>
            <input type="date" value={periodo.dataInicio} onChange={e => setPeriodo(p => (Object.assign(Object.assign({}, p), { dataInicio: e.target.value })))} className="w-36 px-2 py-1 border rounded"/>
            <span className="text-gray-500">até</span>
            <input type="date" value={periodo.dataFim} onChange={e => setPeriodo(p => (Object.assign(Object.assign({}, p), { dataFim: e.target.value })))} className="w-36 px-2 py-1 border rounded"/>
          </div>
        ...
        <div className="overflow-x-auto rounded-xl shadow bg-white animate-fade-in">
          <table className="min-w-full text-sm text-gray-700">
            ...
            <tbody>
              {loadingAtivos ? (<tr><td colSpan={8} className="text-center py-8">Carregando...</td></tr>) : assinantesFiltrados.length === 0 ? (<tr><td colSpan={8} className="text-center py-8 text-gray-400">Nenhum assinante encontrado.</td></tr>) : (assinantesFiltrados.map(a => (<tr key={a.id} className="border-b hover:bg-gray-50 transition-all">
                    <td className="px-4 py-2">{a.nome}</td>
                    <td className="px-4 py-2">{a.plano}</td>
                    <td className="px-4 py-2 font-bold text-blue-700">
                      {typeof a.valor === 'number' && !isNaN(a.valor)
                ...
                : 'R$ 0,00'}
                    ...
                    <td className="px-4 py-2 text-center">
                      <button className="bg-gray-100 hover:bg-gray-200 rounded p-2 transition-all" title={a.origem === 'dinheiro' && a.status === 'AGUARDANDO_PAGAMENTO' ? 'Confirmar pagamento' : 'Ver detalhes'} onClick={() => {
        ...
        {modalConfirmar.open && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md mx-2 animate-fade-in">
              ...
              <p className="mb-4">Deseja confirmar o pagamento do cliente <b>{(_a = modalConfirmar.assinatura) === null || _a === void 0 ? void 0 : _a.nome}</b>?</p>
