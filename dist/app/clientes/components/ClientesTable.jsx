...
export default function ClientesTable({ filtros, reload }) {
    ...
    return (<div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-md p-4 md:p-8 transition-colors duration-300">
      {loading ? (<div className="flex justify-center items-center h-32">
          <Spinner label="Carregando clientes..."/>
        </div>) : clientes.length === 0 ? (<div className="text-center text-gray-400 dark:text-gray-500 py-8">Nenhum cliente encontrado.</div>) : (<>
          <div className="overflow-x-auto">
            <Table aria-label="Tabela de clientes" removeWrapper className="min-w-[700px]">
              ...
              <TableBody>
                {clientesPagina.map((cliente) => (<TableRow key={cliente.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">
                    ...
                    <TableCell>
                      <Tooltip content={cliente.tipo === "assinante" ? "Assinatura ativa" : "Sem assinatura ativa"} placement="top" className="text-xs">
                        <span className={cliente.tipo === "assinante"
                    ...
                    : "inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold bg-zinc-100 text-zinc-700 shadow hover:shadow-md transition-all duration-200 cursor-pointer"} tabIndex={0}>
                          {cliente.tipo === "assinante" ? (<StarIcon className="w-4 h-4 text-yellow-300 drop-shadow"/>) : (<UserIcon className="w-4 h-4 text-zinc-400"/>)}
                          {cliente.tipo === "assinante" ? "Assinante" : "Avulso"}
                        </span>
                      </Tooltip>
                    </TableCell>
                    <TableCell>
                      <button onClick={() => handleEditarCliente(cliente)} className="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-sm font-medium text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-zinc-800 transition-colors" title="Editar cliente">
          ...
          <div className="flex justify-end mt-4">
            <Pagination total={totalPages} page={pagina} onChange={setPagina} showControls size="sm" className="dark:text-zinc-100"/>
          </div>
          {/* Modal de edição */}
          <ClienteModal open={modalAberto} onClose={handleFecharModal} onSuccess={handleSucessoEdicao} cliente={clienteEditando}/>
        </>)}
    </div>);
}
