...
export async function verificarMetasBatidas(mes, ano, unidade) {
    ...
    try {
        ...
        for (const meta of metas) {
            ...
            if (vendas) {
                const metaBatida = vendas.total_vendas_produtos >= meta.meta_venda_produto &&
...
async function calcularVendasBarbeiro(barbeiroId, mes, ano) {
    try {
        ...
        const totalVendasProdutos = (vendasProdutos === null || vendasProdutos === void 0 ? void 0 : vendasProdutos.reduce((sum, venda) => sum + (venda.valor_total || 0), 0)) || 0;
        const totalFaturamento = (vendasServicos === null || vendasServicos === void 0 ? void 0 : vendasServicos.reduce((sum, servico) => sum + (servico.valor_servico || 0), 0)) || 0;
        return {
...
export async function calcularBonificacao(barbeiroId, mes, ano, unidade) {
    const tabelaMetas = unidade === 'barberbeer' ? 'metas_barberbeer' : 'metas_trato';
    try {
...
export async function aplicarBonificacoesFechamento(mes, ano, unidade) {
    try {
...
async function salvarBonificacao(barbeiroId, mes, ano, valor, unidade) {
    try {
...
export async function buscarMetasBarbeiro(barbeiroId, unidade) {
    const tabelaMetas = unidade === 'barberbeer' ? 'metas_barberbeer' : 'metas_trato';
    try {
