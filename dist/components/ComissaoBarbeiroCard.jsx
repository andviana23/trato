...
export function ComissaoBarbeiroCard({ nome, avatarUrl, minutos, percentual, comissao, ticketMedio, tipos, tipoServicoIcone, onClick }) {
    return (<div className="rounded-2xl shadow-md bg-white p-6 flex flex-col items-center border border-gray-100 hover:shadow-xl transition-shadow mb-2">
      <Avatar src={avatarUrl} name={nome} size="lg" className="mb-2 shadow"/>
      <button className="font-bold text-lg text-blue-900 bg-transparent border border-transparent rounded-lg px-3 py-1 mb-1 focus:outline-none focus:ring-2 focus:ring-blue-400 hover:bg-gray-100 transition-all" onClick={onClick} type="button" title="Ver detalhes da comissão" tabIndex={0} style={{ display: 'inline-block', textAlign: 'center' }}>
        {nome}
      </button>
      ...
      <div className="w-full border-t border-gray-100 pt-3 mt-2">
        <div className="grid grid-cols-4 gap-1 text-center">
          {Object.entries(tipos).map(([tipo, qtd]) => (<div key={tipo} className="flex flex-col items-center min-w-0">
