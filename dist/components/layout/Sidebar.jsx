...
import { HomeIcon, UserGroupIcon, CalendarIcon, CurrencyDollarIcon, ChartBarIcon, Cog6ToothIcon, ClipboardDocumentListIcon, UsersIcon, WrenchScrewdriverIcon, CubeIcon, ChevronDownIcon } from "@heroicons/react/24/outline";
...
const adminNavigationBarberBeer = [
...
const recepcionistaNavigationBarberBeer = [
    { name: "Lista da Vez", href: "/lista-da-vez", icon: UsersIcon },
    { name: "Clientes", href: "/clientes", icon: UserGroupIcon },
    { name: "Agendamentos", href: "/dashboard/agendamentos", icon: CalendarIcon },
    // Adicione aqui menus exclusivos da BarberBeer, se houver
];
const recepcionistaNavigationTrato = [
    { name: "Lista da Vez", href: "/lista-da-vez", icon: UsersIcon },
    { name: "Clientes", href: "/clientes", icon: UserGroupIcon },
    { name: "Agendamentos", href: "/dashboard/agendamentos", icon: CalendarIcon },
    // Adicione aqui menus exclusivos da Trato de Barbados, se houver
...
function SidebarSectionTitle({ children }) {
    return (<div className="text-[11px] font-extrabold text-blue-700 dark:text-blue-300 uppercase mb-2 mt-4 px-3 tracking-widest letter-spacing-wider opacity-80">
      {children}
    </div>);
}
export default function Sidebar({ isCollapsed, mobileOpen, onMobileClose, headerSlot }) {
    ...
    const renderNavItem = (item, isActive) => (<Link key={item.name} href={item.href} className={`
    ...
    return (<>
      ...
      <aside className={`
        ...
        <div className="flex flex-col items-center py-4 px-2 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-[#171717]">
          {!isCollapsed && (<span className="text-xl font-bold text-gray-900 dark:text-blue-100 mb-2 text-center">{unidade}</span>)}
          <div className="w-full flex justify-center items-center">
            <select className="border rounded px-3 py-1 text-xs font-bold text-center bg-white dark:bg-[#23272d] text-gray-800 dark:text-blue-200 focus:ring-2 focus:ring-blue-400 h-8" value={unidade} onChange={e => setUnidade(e.target.value)} style={{ minWidth: 130 }} aria-label="Selecionar unidade">
        ...
        <div className="sidebar-menu overflow-y-auto max-h-screen min-h-0">
          <nav className="px-2 py-4 space-y-6 bg-white dark:bg-[#23272d] flex-1 flex flex-col">
            ...
            <div>
              ...
              {navigationItems.map((item) => (<Link key={item.name} href={item.href} className={`
            ...
            <div>
              ...
              <button onClick={() => handleSubmenuToggle('assinaturas')} className={`flex items-center w-full px-3 py-2 rounded-lg font-medium text-[15px] transition-colors duration-200 ${openSubmenu === 'assinaturas' ? 'bg-blue-100 text-blue-700 dark:bg-blue-700 dark:text-white' : 'text-gray-800 dark:text-gray-300 hover:bg-blue-50 dark:hover:bg-[#23272d] hover:text-blue-700 dark:hover:text-white'}`} aria-expanded={openSubmenu === 'assinaturas'} aria-controls="submenu-assinaturas">
                <ChartBarIcon className="w-5 h-5 mr-2"/>
                {!isCollapsed && <span className="truncate flex-1">Assinaturas</span>}
                {!isCollapsed && (<span className={`ml-2 transition-transform ${openSubmenu === 'assinaturas' ? 'rotate-180' : ''}`}>{openSubmenu === 'assinaturas' ? '▲' : '▼'}</span>)}
              </button>
              {!isCollapsed && openSubmenu === 'assinaturas' && (<ul id="submenu-assinaturas" className="pl-8 pr-1 py-2 space-y-1 bg-[#f8fafc] dark:bg-[#1b1e23] rounded-md mt-1 mb-2 border border-gray-200 dark:border-[#23272d]">
                  {assinaturaSubmenu.map((item) => (<li key={item.href}>
                      <Link href={item.href} className={`block px-3 py-2 rounded text-[14px] font-medium transition-colors
                          ${pathname === item.href
            ...
            <div>
              ...
              <button onClick={() => handleSubmenuToggle('cadastros')} className={`flex items-center w-full px-3 py-2 rounded-lg font-medium text-[15px] transition-colors duration-200 ${openSubmenu === 'cadastros' ? 'bg-blue-100 text-blue-700 dark:bg-blue-700 dark:text-white' : 'text-gray-800 dark:text-gray-300 hover:bg-blue-50 dark:hover:bg-[#23272d] hover:text-blue-700 dark:hover:text-white'}`} aria-expanded={openSubmenu === 'cadastros'} aria-controls="submenu-cadastros">
                <ClipboardDocumentListIcon className="w-5 h-5 mr-2"/>
                {!isCollapsed && <span className="truncate flex-1">Cadastros</span>}
                {!isCollapsed && (<span className={`ml-2 transition-transform ${openSubmenu === 'cadastros' ? 'rotate-180' : ''}`}>{openSubmenu === 'cadastros' ? '▲' : '▼'}</span>)}
              </button>
              {!isCollapsed && openSubmenu === 'cadastros' && (<ul id="submenu-cadastros" className="pl-8 pr-1 py-2 space-y-1 bg-[#f1f5f9] dark:bg-[#1b1e23] rounded-md mt-1 mb-2 border border-gray-200 dark:border-[#23272d]">
                  {(unidade === 'BARBER BEER SPORT CLUB' ? cadastrosSubmenuBarberBeer : cadastrosSubmenuTrato).map((item) => (<li key={item.href}>
                      <Link href={item.href} className={`block px-3 py-2 rounded text-[14px] font-medium transition-colors
                          ${pathname === item.href
                    ? 'bg-blue-500 text-white'
                    : 'text-gray-800 dark:text-gray-200 hover:bg-blue-100 dark:hover:bg-blue-800 hover:text-blue-700 dark:hover:text-white'}
            ...
            <div>
              <button className={`flex items-center w-full px-4 py-2 rounded-lg font-semibold text-base transition ${openSubmenu === 'distribuicao' ? 'bg-blue-100 text-blue-700' : 'text-gray-700 hover:bg-blue-50'}`} onClick={() => setOpenSubmenu(openSubmenu === 'distribuicao' ? null : 'distribuicao')}>
                <ChartBarIcon className="w-5 h-5 mr-2"/>
              ...
              {openSubmenu === 'distribuicao' && (<ul id="submenu-distribuicao" className="pl-8 pr-1 py-2 space-y-1 bg-[#f8fafc] dark:bg-[#1b1e23] rounded-md mt-1 mb-2 border border-gray-200 dark:border-[#23272d]">
                  {(unidade === 'BARBER BEER SPORT CLUB' ? distribuicaoSubmenuBarberBeer : distribuicaoSubmenuTrato).map((item) => (<li key={item.href}>
                      <Link href={item.href} className={`block px-3 py-2 rounded text-[14px] font-medium transition-colors
                          ${pathname === item.href
                    ? 'bg-blue-500 text-white'
                    : 'text-gray-800 dark:text-gray-200 hover:bg-blue-100 dark:hover:bg-blue-800 hover:text-blue-700 dark:hover:text-white'}
      ...
      {mobileOpen && (<>
          <div className="fixed inset-0 bg-gray-900/50 z-40 md:hidden" onClick={onMobileClose}/>
          <aside className="bg-gradient-to-br from-[#0f2233] to-[#112d42] w-60 min-h-screen flex flex-col">
            ...
            <div className="flex flex-col items-center py-6 border-b border-cyan-700">
              ...
              <span className="text-white font-bold">{(profile === null || profile === void 0 ? void 0 : profile.name) || 'Usuário'}</span>
